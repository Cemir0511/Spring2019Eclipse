...
public class ReverseArraYListWithForLoop {
...
//	    lst1.set(5 - 2 , temp) ;
       ...
       //swap first item with last item
        ...
        Integer temp = lst1.get(0);   // storing firt item value in temp
        lst1.set(0, lst1.get(lst1.size()-1)); // updating first item value to last item
        lst1.set(lst1.size()-1, temp); //updating last item value to original first item value
        ...
        //swapping 2nd itme with 4th item
        temp = lst.get(1);
        lst1.set(1, lst1.get(5-2));
        lst1.set(5-2, temp);
        ...
        for (int i=0;i<lst1.size()/2; i++ ) {
        	...
        	Integer temp = lst1.get(i);   // storing firt item value in temp
            lst1.set(0, lst1.get(lst1.size()-1)); // updating first item value to last item
            lst1.set(lst1.size()-1, temp); //updating last item value to original first item value	
